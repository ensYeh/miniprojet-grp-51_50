...
public class NoteManager {
  ...
  public static List<String> getNotesForNumber(final int number, final String path) {
    try {
      ...
      System.err.println("Erreur lors de la lecture du fichier \"" + FILE_NAME + "\": "
              + e.getMessage());
    }
   ...
   *
   * @param path Le chemin du répertoire dans lequel se trouve le fichier de notes.
  ...
  public static boolean checkNotesFile(String path) {
    ...
    try {
      ...
      System.err.println("Erreur lors de la création du fichier \"" + FILE_NAME + "\": "
              + e.getMessage());
    }
    return fileCreated;
  }
  ...
  public static void modifyNoteNumber(Integer X, String path, String val) {
    try {
      ...
      System.err.println("Erreur lors de la modification du fichier \"" + FILE_NAME + "\": "
              + e.getMessage());
    }
  }
   ...
   * @param currentDir Le répertoire actuel dans lequel se trouve le fichier de notes.
   */
  ...
  public static void deleteNoteIfExists(int number, String chemin) {
    try {
      ...
      System.err.println("Erreur lors de la suppression de la note dans le fichier \""
  ...
  public static boolean checkAndDeleteEmptyNotesFile(String path) {
    try {
      ...
      System.err.println("Erreur lors de la vérification et suppression du fichier \""
              + FILE_NAME + "\": " + e.getMessage());
      return false;
    }
  }
  ...
  public static void writeNotesToJson(Path filePath, List<NoteEntry> noteEntries)
          throws IOException {
    Gson gson = new GsonBuilder().setPrettyPrinting().create();
    String jsonContent = gson.toJson(noteEntries);
    // Écrire le contenu JSON dans le fichier avec un format correct
    Files.write(filePath, jsonContent.getBytes(), StandardOpenOption.WRITE,
