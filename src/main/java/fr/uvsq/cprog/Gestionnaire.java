    ...
    public void copy(int number) {
        if (number >= 0 && number < files.size()) {
            ...
            if (!element.isDirectory()) {
                ...
                FileElement newFileCopy = new FileElement(number, element.getContent(), newName);
    ...
    public void past(int number) {
        if (number >= 0 && number < files.size()) {
            FileElement element = files.get(number);
                ...
                boolean nameExists = files.stream().anyMatch(file -> file.getName().equals(newName));
                if (nameExists) {
                    // Concaténer "-copy" au nom seulement si l'élément existe déjà
                    final String finalNewName = newName + "-copy";

                    FileElement newFileCopy = new FileElement(number, element.getContent(), finalNewName);
                    files.add(newFileCopy);
